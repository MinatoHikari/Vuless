...
export default defineComponent({
    ...
    setup(props, { slots }) {
        ...
        const getGridColumnsAndRows: (param: ColumnsAndRows) => string = (param) => {
        ...
        const getAllStyles = () => {
            ...
            if (props.type === 'grid' && props.grid) {
                ...
                gridStyle.gridTemplateColumns = props.grid.columns
                    ? getGridColumnsAndRows(props.grid.columns)
                    : gridStyle.gridTemplateColumns;
                gridStyle.gridTemplateRows = props.grid.rows
                    ? getGridColumnsAndRows(props.grid.rows)
                    : '';
                gridStyle.gridTemplateAreas = props.grid.area ? getGridArea(props.grid.area) : '';
                gridStyle.gridAutoColumns = props.grid.columns
                    ? getGridAutoColumnsAndRows(props.grid.columns)
                    : '';
                gridStyle.gridAutoRows = props.grid.rows
                    ? getGridAutoColumnsAndRows(props.grid.rows)
                    : '';
                if (props.grid.flow)
                    classes.push(`vl-container-grid-flow-${getGridFlowClassName(props.grid.flow)}`);
...
<style lang="scss">
$flowList: row, column;
@each $flow in $flowList {
    .vl-container-grid-flow-#{$flow} {
        grid-auto-flow: $flow;
    }
    .vl-container-grid-flow-#{$flow}-dense {
        grid-auto-flow: $flow dense;
    }
}

$placeList: start, end, center, stretch;
@each $place in $placeList {
    .vl-container-grid-place-items-#{$place} {
        place-items: $place;
    }
    @each $columnPlace in $placeList {
        .vl-container-grid-place-items-#{$place}-#{$columnPlace} {
            place-items: $place $columnPlace;
        }
    }
}

$contentList: append($placeList, space-around space-between space-evenly);
@each $content in $contentList {
    .vl-container-grid-place-content-#{$content} {
        place-content: $content;
    }
    @each $columnContent in $contentList {
        .vl-container-grid-place-content-#{$content}-#{$columnContent} {
            place-content: $content $columnContent;
        }
    }
}
</style>
